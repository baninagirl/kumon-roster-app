...
function buildPoolConfig() {
  ...
  if (connectionString) {
    ...
    //
    // `max` (Aug 28, Vercel go-live follow-up): on Vercel, every concurrent
    // request can land on its own separate serverless function instance,
    // each running this same file and each creating its OWN Pool -- so the
    // real number of Postgres client connections in flight at once is
    // (concurrent instances) x (this `max`), not just `max`. `pg`'s default
    // `max` is 10 per Pool, which multiple simultaneous users (the center
    // has ~20 staff who can all have the app open together) can multiply
    // into far more client connections than Supabase's pooler allows --
    // that's what the "max clients reached... pool_size: 15" error earlier
    // was. Supabase's own guidance for serverless is to keep the
    // client-side pool small when going through the Transaction pooler,
    // since the pooler itself (not this local pool) is what actually
    // multiplexes many callers onto a small number of real backend
    // connections. A given request here only ever needs one connection at
    // a time (every DB call in this file is sequentially awaited, nothing
    // opens several connections at once for one request), so a small `max`
...
async function initialize(pool) {
  ...
  //
  // Aug 28 (Vercel go-live follow-up): this used to SELECT every row, then
  // loop in JS and issue one awaited UPDATE per row that needed to change.
  // Against SQLite (same process, no network) that was free; against real
  // Postgres over the network -- and worse, re-run from scratch on every
  // serverless cold start, of which there can be many at once when several
  // staff open the app together -- up to ~1300 sequential round trips here
  // alone was a real contributor to the app hanging on first deploy. The
  // same regex, translated to Postgres's own regex operator, lets one
  // single UPDATE do the same "only touch rows whose flag is actually
  // wrong" work Postgres-side, matching the JS version's semantics exactly
  // (same anchors, same optional [ T]HH:MM:SS suffix, same "only counts as
  // date-like when current_level is empty" condition) but as one query
  // instead of one-or-many.
  await pool.query(`
    UPDATE subject_enrollment
    SET needs_level_review = CASE
      WHEN current_level IS NULL
       AND current_level_raw IS NOT NULL
       AND TRIM(current_level_raw) ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}([ T][0-9]{2}:[0-9]{2}:[0-9]{2})?$'
      THEN 1 ELSE 0
    END
    WHERE needs_level_review IS DISTINCT FROM (CASE
      WHEN current_level IS NULL
       AND current_level_raw IS NOT NULL
       AND TRIM(current_level_raw) ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}([ T][0-9]{2}:[0-9]{2}:[0-9]{2})?$'
      THEN 1 ELSE 0
  ...
  // (re)normalize `grade` from it every startup. Same round-trip problem
  // and same fix as needs_level_review above -- ashr.normalizeGrade() is
  // plain JS (not portable to a single SQL expression), so the computation
  // stays in JS, but every row that actually needs a new value is now
  // written back in one batched UPDATE (via a VALUES list) instead of one
  // round trip per changed row.
  ...
  for (const row of gradeRows) {
    ...
    if (normalized !== row.grade) gradeChanges.push([row.id, normalized]);
  }
  if (gradeChanges.length) {
    const values = gradeChanges.map((_, i) => `($${i * 2 + 1}::int, $${i * 2 + 2}::text)`).join(', ');
    const params = gradeChanges.flat();
    await pool.query(
      `UPDATE student AS s SET grade = c.grade FROM (VALUES ${values}) AS c(id, grade) WHERE s.id = c.id`,
      params
    );
  }
}
